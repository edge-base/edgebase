...
- **Capabilities**: [State (Room State)](/docs/room/state), [Meta (Room Info)](/docs/room/meta), [Members (Presence)](/docs/room/members), and [Signals (Broadcast)](/docs/room/signals) are the primary Room surfaces.
:::
...
Room combines four core real-time capabilities in one primitive:
...
- **`room.signals`** (broadcast) — lightweight pub/sub for chat, cursors, reactions, and other transient events
And the client surface includes two companion namespaces around those four capabilities:

- **`room.admin`** — moderation and operator controls such as kick, block, and role changes
...
- **Conferencing control planes** — participant tracking, presence, signaling

---

## Choosing Room vs Database Subscriptions vs Push

Choose Room when you need interactive real-time coordination — authoritative state, metadata, presence, or signals. If you only need database change feeds in the UI, use [Database Subscriptions](/docs/database/subscriptions). If you need delivery to devices outside the active app session, use [Push](/docs/push).
...
| `room.admin.*` | Moderation controls such as kick, block, and role changes |
...
<NextStepCards steps={[
  ...
  { icon: "📡", title: "Signals (Broadcast)", description: "Transient room events, chat, and lightweight coordination", href: "signals" },
]} />
