...
edgebase.config.ts  (plugins: [stripePlugin({...})])
  ...
  → wrangler deploy
  → first relevant runtime request / scheduled trigger
  → executePluginMigrations()  (reconciles pending plugin migrations once per isolate)
```

**Low runtime overhead**: Plugin code is part of the Worker bundle. The runtime does not scan for plugins or dynamically import them; it executes the already-registered tables/functions and lazily reconciles plugin migrations when needed.
...
└──────────────────────┬──────────────────────────┘
          ...
          ┌────────────┼────────────┐
          ▼            ▼            ▼
   mergePluginTables  initFunctionRegistry   wrangler deploy
   (tables → DB)      (functions + auth/
                       storage hooks)
          │            │            │
          └────────────┼────────────┘
                       ▼
              ...
              — runtime lazily reconciles plugin migrations
                before plugin-capable routes and scheduled runs
```
