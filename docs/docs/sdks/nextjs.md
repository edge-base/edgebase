...
```bash
npm install @edgebase/web @edgebase/ssr
```
...
For client-side components (`'use client'`), use `@edgebase/web`:
...
For Server Components and Route Handlers, use `@edgebase/ssr`:
...
import { increment } from '@edgebase/core';
