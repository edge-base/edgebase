...
- the Unreal plugin supports only Win64, macOS, and Linux until matching native archives are shipped; never allow another platform to fall through as `EDGEBASE_HAS_CORE=0`
- `ThirdParty/` must contain and link both EdgeBase core and IXWebSocket archives, and the metadata-free standalone consumer gate must remain green
