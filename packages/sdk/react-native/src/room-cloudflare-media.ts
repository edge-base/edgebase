...
export interface RoomCloudflareKitParticipant {
  id: string;
...
export interface RoomCloudflareKitSelf {
  id: string;
  audioTrack?: MediaStreamTrack;
  videoTrack?: MediaStreamTrack;
  screenShareTracks?: { audio?: MediaStreamTrack; video?: MediaStreamTrack };
...
type ParticipantListenerSet = {
  participant: RoomCloudflareKitParticipant;
  onAudioUpdate: ParticipantEventHandlers['audioUpdate'];
  onVideoUpdate: ParticipantEventHandlers['videoUpdate'];
  onScreenShareUpdate: ParticipantEventHandlers['screenShareUpdate'];
};
...
function installMessage(packageName: string): string {
...
export class RoomCloudflareMediaTransport implements RoomMediaTransport {
  ...
  private readonly options: Required<Omit<RoomCloudflareMediaTransportOptions, 'clientFactory' | 'mediaDevices'>> & {
    clientFactory?: RoomCloudflareKitClientFactory;
    mediaDevices?: Pick<MediaDevices, 'getUserMedia' | 'getDisplayMedia' | 'enumerateDevices'>;
  };
  ...
  private onParticipantJoined = (participant: RoomCloudflareKitParticipant) => {
    this.attachParticipant(participant);
  };
  private onParticipantLeft = (participant: RoomCloudflareKitParticipant) => {
  ...
  async startScreenShare(
    _constraints: unknown = { video: true, audio: false },
  ...
  private async resolveClientFactory(): Promise<RoomCloudflareKitClientFactory> {
    ...
    this.clientFactoryPromise ??= import('@cloudflare/realtimekit-react-native')
      .then((mod) => (mod.default ?? mod) as RoomCloudflareKitClientFactory)
      .catch((error) => {
  ...
  private getParticipantMap(): ParticipantMapLike<RoomCloudflareKitParticipant> | null {
    if (!this.meeting) {
      return null;
    }
  ...
  private attachParticipant(participant: RoomCloudflareKitParticipant): void {
  ...
  private syncParticipantTracks(participant: RoomCloudflareKitParticipant): void {
