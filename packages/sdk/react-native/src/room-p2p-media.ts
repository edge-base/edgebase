...
type DisplayCaptureConstraints = unknown;
...
export class RoomP2PMediaTransport implements RoomMediaTransport {
  ...
  private readonly options: Required<Omit<RoomP2PMediaTransportOptions, 'mediaDevices' | 'peerConnectionFactory'>> & {
    peerConnectionFactory?: (configuration: RTCConfiguration) => RTCPeerConnection;
    mediaDevices?: Pick<MediaDevices, 'getUserMedia' | 'getDisplayMedia'>;
  };
  ...
  private runtimePromise: Promise<ReactNativeWebRTCModule> | null = null;
  ...
  async startScreenShare(
    ...
    const stream = await devices.getDisplayMedia(constraints as never);
  ...
  private ensurePeer(memberId: string): P2PPeerState {
    ...
    const factory = this.options.peerConnectionFactory ?? ((configuration: RTCConfiguration) => new RTCPeerConnection(configuration));
  ...
  private async ensureRuntime(): Promise<ReactNativeWebRTCModule> {
    this.runtimePromise ??= import('@cloudflare/react-native-webrtc')
      .then((mod) => {
        const runtime = mod as unknown as ReactNativeWebRTCModule;
        runtime.registerGlobals?.();
        return runtime;
      })
      .catch((error) => {
        throw new Error(`${installMessage('@cloudflare/react-native-webrtc')}\n${String(error)}`);
      });
    return this.runtimePromise;
  }

  private async resolveMediaDevices(): Promise<Pick<MediaDevices, 'getUserMedia' | 'getDisplayMedia'> | undefined> {
    if (this.options.mediaDevices) {
      return this.options.mediaDevices;
    }

    const runtime = await this.ensureRuntime();
    return runtime.mediaDevices
