# Changelog

## 0.5.0 (2026-07-25)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata, generated API surfaces, and dependency bounds with EdgeBase 0.5.0.
- No Python-specific API compatibility break is introduced.

## 0.4.9 (2026-07-17)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.4.9.
- No public Python API compatibility break is introduced.

## 0.4.8 (2026-07-16)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.4.8.
- No public Python API compatibility break is introduced.

## 0.4.7 (2026-07-15)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.4.7.
- No public Python API compatibility break is introduced.

## 0.4.6 (2026-07-14)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.4.6.
- No public Python API compatibility break is introduced.

## 0.4.5 (2026-07-14)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.4.5.
- No public Python API compatibility break is introduced.

## 0.4.4 (2026-07-13)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.4.4.
- No public Python API compatibility break is introduced.

## 0.4.3 (2026-07-13)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.4.3.
- No public Python API compatibility break is introduced.

## 0.4.2 (2026-07-12)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.4.2.
- No public Python API compatibility break is introduced.

## 0.4.1 (2026-07-12)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.4.1.
- No public Python API compatibility break is introduced.

## 0.4.0 (2026-07-11)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.3.9.
- No public Python API compatibility break is introduced.

## 0.3.8 (2026-07-10)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.3.8.
- No public Python API compatibility break is introduced.

## 0.3.7 (2026-07-10)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.3.7.
- No public Python API compatibility break is introduced.

## 0.3.6 (2026-07-10)

### Changed

- Synchronized `edgebase-core`, `edgebase-admin`, and the umbrella SDK package
  metadata and compatible dependency bounds with EdgeBase 0.3.6.
- No public Python API compatibility break is introduced.

## 0.1.0 (2026-02-13)

### Features (Dart/Swift/Kotlin SDK 완전 패리티)
- **Core**: EdgeBase 클라이언트, httpx 비동기 HTTP, TokenManager (asyncio.Lock, 30s 버퍼)
- **Auth**: sign_up/sign_in/sign_out/sign_in_anonymously/OAuth/link, on_auth_state_change (콜백)
- **AdminAuth**: Service Key 기반 유저 관리
- **Collection**: 불변 쿼리 빌더, CRUD, batch (create_many/upsert_many/update_many/delete_many)
- **DocRef**: get/update/delete/on_snapshot (async generator)
- **Storage**: upload/download/delete/list/get_url/get_metadata/update_metadata/signed_url/copy/move/resumable
- **DatabaseLive**: websockets + auto-reconnect, Presence/Broadcast 채널
- **FieldOps**: increment/delete_field 원자적 연산
- **TokenStorage**: MemoryTokenStorage + KeyringTokenStorage (macOS/Linux/Windows + 환경변수 폴백)
