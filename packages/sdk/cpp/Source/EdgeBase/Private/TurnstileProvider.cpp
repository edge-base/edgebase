...
bool percentDecodeMessage(const std::string &encoded, std::string &decoded) {
  ...
  for (std::size_t i = 0; i < encoded.size(); ++i) {
    ...
    if (decoded.size() > kMaxRawMessageBytes) return false;
  }
  return true;
}
...
std::string TurnstileProvider::fetchSiteKey(
  ...
  const auto now = clock();
  {
    ...
    if (cached != siteKeyCache_.end()) siteKeyCache_.erase(cached);
  }
  ...
  try {
    ...
    throw std::runtime_error("captcha-unavailable: config_fetch_failed");
  }
  if (!result.ok) {
    throw std::runtime_error("captcha-unavailable: config_fetch_failed");
  }
  if (result.body.size() > 65536) {
    throw std::runtime_error("captcha-unavailable: config_invalid_response");
  }
  ...
  if (json["captcha"].is_null()) return "";
...
std::string TurnstileProvider::resolveCaptchaToken(
  ...
  if (!manualToken.empty()) return manualToken;
#if !EDGEBASE_HAS_CORE
  (void)http;
  (void)action;
  return "";
  ...
  if (guard) guard();
  if (!http || fetchSiteKey(http).empty()) return "";
...
std::string TurnstileProvider::acquireCaptchaToken(
  ...
  const std::string challengeUrl = buildChallengeUrl(baseUrl, action, channel);
...
std::string TurnstileProvider::buildChallengeUrl(
  ...
  if (origin.empty() || !isAllowedAction(action) ||
    ...
    throw std::invalid_argument(
        "Turnstile requires an origin-only HTTPS URL, fixed action, and secure channel");
  }
...
bool TurnstileProvider::tryParseChallengeUri(
  ...
  if (uri.compare(0, std::char_traits<char>::length(kMessagePrefix),
                  kMessagePrefix) != 0) return false;
  std::string decoded;
  if (!percentDecodeMessage(
          uri.substr(std::char_traits<char>::length(kMessagePrefix)), decoded)) {
    return false;
  }
