<script lang="ts">
  /**
   * Auth Analytics — authentication metrics dashboard.
   * Shows signup/signin rates, OAuth provider distribution, error rates.
   */
...
<PageShell title="Auth Analytics" description="Authentication and authorization metrics">
  ...
  <div class="analytics-grid">
    <MetricCard label="Auth Requests" value={data?.summary?.totalRequests ?? 0} {loading} />
  ...
  <div class="analytics-chart">
    <TimeChart data={chartData()} type="line" height={240} label="Auth requests over time" formatTime={chartTimeFormatter} {loading} color="#7c3aed" />
  </div>

  <div class="analytics-bottom">
    <DistributionBar items={breakdownItems()} title="Action Distribution (signup, signin, oauth...)" {loading} />
    <TopList items={topItems()} title="Top Auth Operations" {loading} />
  </div>
</PageShell>
