<script lang="ts">
  /**
   * Database Analytics — CRUD operation metrics dashboard.
   * Shows read/write distribution, table usage, slow queries.
   */
  ...
  async function fetchAnalytics() {
    try {
      const res = await api.fetch<AnalyticsData>(`data/analytics?range=${range}&category=db&metric=overview`);
      data = res;
    } catch (err) {
      toastError(err instanceof Error ? err.message : 'Failed to load database analytics');
    } finally {
      loading = false;
    }
  }
...
<PageShell title="Database Analytics" description="CRUD operations and table usage metrics" docsHref={adminDashboardAnalyticsDocs}>
  {#snippet actions()}
  ...
  <div class="analytics-grid">
    <MetricCard label="DB Queries" value={data?.summary?.totalRequests ?? 0} {loading} />
    <MetricCard label="Active Users" value={data?.summary?.uniqueUsers ?? 0} {loading} />
  ...
  <div class="analytics-chart">
    <TimeChart data={chartData()} type="bar" height={240} label="Database operations over time" formatTime={chartTimeFormatter} {loading} color="#16a34a" />
  </div>

  <div class="analytics-bottom">
    <DistributionBar items={breakdownItems()} title="Operation Distribution (CRUD)" {loading} />
    <TopList items={topItems()} title="Most Accessed Tables" {loading} />
  </div>
</PageShell>
