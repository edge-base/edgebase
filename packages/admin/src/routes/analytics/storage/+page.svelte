<script lang="ts">
  ...
  async function fetchAnalytics() {
    try {
      ...
      toastError(err instanceof Error ? err.message : 'Failed to load storage analytics');
    } finally {
      loading = false;
    }
  }
...
<PageShell title="Storage Analytics" description="File upload, download, and bucket usage metrics">
  {#snippet actions()}
